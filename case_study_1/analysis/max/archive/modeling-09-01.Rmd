...
allErrors = foreach(numAngles = allAngles
                    ,.combine = rbind
...
```{r}
library('caret')
t=as.data.frame(allErrorsLong)
t$RMSESD  =sd (t$RMSE)
tBest = best(t,"RMSE",maximize=FALSE)
message("Best: Angles: ",t[tBest,'NumAngles'],", Neighbors: ",t[tBest,'NumNeighbors'],", RMSE: ", t[tBest,'RMSE'])
tOneSE = oneSE(t,"RMSE",maximize=FALSE,num=5)
message("oneSL: Angles: ",t[tOneSE,'NumAngles'],", Neighbors: ",t[tOneSE,'NumNeighbors'],", RMSE: ", t[tOneSE,'RMSE'])
```

```{r}
p = ggplot(allErrorsLong, aes(x=NumNeighbors, y=NumAngles, fill= RMSE)) + 
  geom_tile() +
  scale_y_continuous(breaks=allAngles) +
  scale_fill_distiller(palette = "RdYlBu") +
  #scale_fill_distiller(palette = "Blues",direction=0) +
  geom_point(data=t[tBest,],shape='B',size=7) +
  geom_point(data=t[tOneSE,],shape='O',size=7) 
p
