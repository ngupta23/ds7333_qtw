...
run_kfold = function(K, v, offline, onlineCVSummary, folds, numAngles, weighted=FALSE){
  ...
  for (j in 1:v) {
    ...
    for(k in 1:K){ 
      ...
      errCV[k] =  calcError(estSingleK, actualFold) #returing all folds
    }
    allErr=rbind(allErr,data.frame('fold'=j, 'numNeighbors' = 1:K,'RMSE' = errCV))
  } 
...
### functions for CV and best Model
...
get_CV = function(K,v,offline,onlineCVSummary,permuteLocs,numAngles,weighted = TRUE){
  ...
  allErrorsCV = foreach(numAngles = allAngles
                        ...
                        ,.export = c('run_kfold','predXYallK','reshapeSS','findNN','calcError'
                                     ,'numMacs','selectTrain','roundOrientation') #functions to be used in the parallel processes
  ) %dopar% {
...
find_best = function(allErrorsCV){
  ...
  allErrors = allErrorsCV %>%
    ...
    ungroup() %>%
    mutate(RMSESD=sd(RMSE)
           ,best=FALSE
           ,oneSE=FALSE)
  allErrors[best(as.data.frame(allErrors),"RMSE",maximize=FALSE),]$best=TRUE
  allErrors[oneSE(as.data.frame(allErrors),"RMSE",maximize=FALSE,num=30),]$oneSE=TRUE
  return(allErrors)
}

plot_best = function(allErrors) {
  p = ggplot(allErrors, aes(x=numNeighbors, y=numAngles, fill= RMSE
                         , text=paste0("A:",numAngles," N:",numNeighbors," R:", round(RMSE,1)))) + 
    geom_tile() +
    scale_y_continuous(breaks=allAngles) +
    #scale_fill_distiller(palette = "RdYlBu") +
    scale_fill_gradient2(low = "green",mid='darkorange', high = "darkred", na.value = NA
                         ,midpoint=mean(c(max(allErrors$RMSE),min(allErrors$RMSE)))
                         #,midpoint=median(Errors$RMSE)
    )+
